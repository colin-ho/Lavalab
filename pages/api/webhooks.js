  ...
  if (req.method === 'POST') {
    ...
    if (event.type === 'checkout.session.completed') {
        try {
            ...
            batch.set(customerSub, { subscriptionId:metadata.subscriptionId,boughtAt: serverTimestamp(),stripeSubscriptionId:event.data.object.subscription,redemptionCount:0,redeemedAt:[],redeeming:false });
            await batch.commit();
          } catch (err) {
            console.log(`❌ Error message: ${err.message}`);
            res.status(400).send(`Webhook Error: ${err.message}`);
