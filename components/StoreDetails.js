...
const initialState = {'mon':{'open':{'hr':0,'min':0},'close':{'hr':0,'min':0}},
...
export default function StoreDetails({business}) {
    ...
    const submitDetails = async ({phone,email,website}) =>{
        ...
        if(businessRef){
            ...
            await businessRef.update({phone: phone,email:email,website:website,address: address,
...
function Delay({isOpen,onClose,setDelay,delay}) {
    ...
    return (
        <Modal isCentered isOpen={isOpen} onClose={onClose}>
          <ModalOverlay />
          <ModalContent>
            <ModalHeader>Delay Fulfillment</ModalHeader>
            <ModalCloseButton /> 
            <ModalBody >
                <RadioGroup onChange={setValue} value={value}>
                <VStack align="flex-start" mt="-20px" mb="20px" spacing="4">
                <Text>Add a delay to redemption ETAs</Text>
