...
export default function Home({displayName, subscriptions,redemptions}){
    ...
    useEffect(() => {
        ...
        if(user&&joined&&subscriptions.length>0){
            ...
            unsubscribe = firestore.collectionGroup('subscribedTo').where('subscriptionId','in',ids).orderBy('boughtAt').onSnapshot((snapshot)=>{
                let tempData = [];
                let money = 0;
                tempData.push({date:joined,value:0})
                snapshot.forEach((doc)=>{
                    money+= parseInt(subscriptions.filter((sub)=>doc.data().subscriptionId == sub.id)[0].price); 
        ...
        return unsubscribe;
    }, [subscriptions,redemptions,user,joined])
    ...
    return(
    <Box>
        <Heading  size="lg" mb="10px"> Welcome Back</Heading>{console.log(data)}
